...
class TestEvaluator(unittest.TestCase):
    ...
    def test_evaluate_check_constraint(self):
        ...
        history = [[{"m2", "m4"}, {"m1", "m3", "m5"}], [{"m1", "m3"}, {"m2", "m4"}]]
        num_remaining_members_in_each_team = [0, 1, 0]
        ev = evaluator.Evaluator({}, history, num_remaining_members_in_each_team)
        v = ev.evaluate(state)
    ...
    def test_evaluate_check_history_check(self):
        ...
        history = [[{"m2", "m4"}, {"m1", "m5"}, {"m3"}], [{"m1"}, {"m4"}, {"m2", "m3"}]]
        num_remaining_members_in_each_team = [0, 0, 0]
        ev = evaluator.Evaluator({}, history, num_remaining_members_in_each_team)
        v = ev.evaluate(state)
